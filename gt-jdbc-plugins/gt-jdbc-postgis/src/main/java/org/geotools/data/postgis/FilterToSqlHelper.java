...
class FilterToSqlHelper {
    ...
    /**
     * Performs custom visits for functions that cannot be encoded as <code>functionName(p1, p2, ... pN).</code>
     *
     * @param function
     * @param extraData
     * @return
    ...
    Expression getParameter(Function function, int idx, boolean mandatory) {
        ...
        if (params == null || params.size() <= idx) {
            if (mandatory) {
                throw new IllegalArgumentException("Missing parameter number " + (idx + 1) + "for function "
                        + function.getName() + ", cannot encode in SQL");
            }
        }
