...
/**
 * This dialect uses the Oracle 10 specific sdoapi.jar API, which is touted to be faster than then standard JGeometry
 * one. Benchmarking shows that it's no match for the standard GeometryConverter thought, and only adds a nasty
 * dependency that cannot be redistributed, so it lays here inactive only for historical purposes. Un comment here and
 * in OracleDialect if you want to try it out.
 *
 * @author Andrea Aime
 */
